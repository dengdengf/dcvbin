...
class Trainer(nn.Module):
    ...
    def train(self):
            ...
            if self.curriculum:
                if self.args.epochs >=3:
                    if (epoch >= int(self.args.epochs/3)) & (epoch < int(self.args.epochs/3)+1):
                        ...
                        self.model.module.dnabert2.load_state_dict(torch.load(load_dir+'/pytorch_model.bin', weights_only=True))
                        self.model.module.contrast_head.load_state_dict(torch.load(load_dir+'/con_weights.ckpt', weights_only=True))
        ...
        lossoutput_file_path = "/fs1/private/user/zhaozhimiao/csm/DNABERT_S-main/train/pretrain/mymodel_output/train_loss.txt"
        with open(lossoutput_file_path, 'w') as f:
        ...
        plt.legend()
        plt.savefig("/fs1/private/user/zhaozhimiao/csm/DNABERT_S-main/train/pretrain/mymodel_output/train_loss.jpg")
        return None
    ...
    def val(self):
        ...
        for step in range(self.args.logging_step, np.min([self.all_iter, self.args.logging_step*self.args.logging_num+1]), self.args.logging_step):
            ...
            if val_loss < best_val_loss:
             ...
             # 绘制验证集损失图
        vallossoutput_file_path = "/fs1/private/user/zhaozhimiao/csm/DNABERT_S-main/train/pretrain/mymodel_output/val_loss.txt"
        with open(vallossoutput_file_path, 'w') as f:
            for epoch, loss in enumerate(epoch_val_losses):
                f.write(f'Epoch {epoch}: {loss}\n')
        ...
        plt.legend()
        plt.savefig("/fs1/private/user/zhaozhimiao/csm/DNABERT_S-main/train/pretrain/mymodel_output/validation_loss.jpg")